...
|  | [ Database Interfaces ](reference_bases.md "Database Interfaces") / [ Trade ](reference_trading.md "Trade") / [ Positions ](trading_position.md "Positions") / [ IMTPosition ](imtposition.md "IMTPosition")/ VolumeGatewayExt | [](imtposition_pricegateway.md "PriceGateway") [](imtposition_actiongateway.md "ActionGateway") |
| --- | --- | --- |
| --- | --- |

IMTPosition::VolumeGatewayExt
Get the volume at which the position is actually routed to an external trading system through the gateway.
C++
UINT64 IMTPosition::VolumeGatewayExt() const  
---  
.NET (Gateway/Manager API)
ulong CIMTPosition.VolumeGatewayExt()  
---  
Return Value
The volume in which the position is actually routed to an external system, in UINT64 format (one unit corresponds to 1/100000000 of a lot, for example, 105000000 means 1.05 lots).
Note
The method operates with [high-precision volumes](features.htm#volume) (8 decimal places).
IMTPosition::VolumeGatewayExt
Set the volume at which the position is actually routed to an external trading system through the gateway.
C++
MTAPIRES IMTPosition::VolumeGatewayExt( const UINT64 volume // position volume )  
---  
.NET (Gateway/Manager API)
MTRetCode CIMTPosition.VolumeGatewayExt( ulong volume // position volume )  
---  
Parameters
volume
[in] The volume in which the position is actually routed to an external system, in UINT64 format (one unit corresponds to 1/100000000 of a lot, for example, 105000000 means 1.05 lots).
Return Value
